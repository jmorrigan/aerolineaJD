...
public class ListarAvion {
     ...
     * @param sillas
     */
    public ListarAvion(ArrayList<Silla> sillas) {
        this.sillas = sillas;
    }
    ...
    public void imprimirAvion(){
          ...
          if (sillas != null) {
            for (Silla opc : sillas) {
                System.out.println("Clase:"+opc.getClase()+" Silla:"+opc.getNumero()+" Cedula:"+opc.getPersona().getCedula()+" Fecha:"+opc.getPersona().getFecha()+" Nombre:"+opc.getPersona().getNombre()+" Apellido:"+opc.getPersona().getApellido()+" Pagó:"+opc.getPrecio());
                }
            }
          else {
